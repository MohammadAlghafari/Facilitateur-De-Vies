...
class UserProfileRepositoryImp implements UserProfileRepository {
  ...
  UserProfileRepositoryImp(this._deleteAccountApiImp,this._getAccountApiImp,this._updateAccountApiImp);

  @override
  Future<Either<dynamic, ApiResponse<String>>> deleteAccount({required String id}) async {
  ...
  @override
  Future<Either<dynamic, ApiResponse<SuccessResponseModel>>> updateAccount({required AccountModel accountModel,required int id}) async {
    ApiResponse<SuccessResponseModel> result;
    try {
      result = await _updateAccountApiImp.updateAccount(accountModel:accountModel,id: id);
      if (result.status == Status.completed) {
        return Right(result);
      } else {
        return Left(result);
